...
from utils import (YOUTUBEDL_BIN, format_bytes,
                   to_string, to_bytes)
...
class DownloadManager(QThread):
    ...
    """
    sig_callafter = pyqtSignal(str)
    sig_worker_callafter = pyqtSignal(dict)
    ...
    def __init__(self, parent, download_list, opt_manager, log_manager=None):
        ...
        for worker in self._workers:
            worker.sig_callafter.connect(self._on_worker_callafter)
    ...
    def run(self):
        ...
        # Join and collect
        for worker in self._workers:
            #worker.join() #QThread do not have join?
            worker.wait()
    ...
    def _on_worker_callafter(self, signal, data):
        self.sig_worker_callafter.emit(data)
...
class Worker(QThread):
    ...
    """
    sig_callafter = pyqtSignal(str, dict)
