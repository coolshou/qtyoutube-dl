...
class PipeReader(QThread):
    ...
    def join(self, timeout=30):
        self._running = False
        #super(PipeReader, self).join(timeout) #Qthread do not have join
        super(PipeReader, self).wait(timeout)


class YoutubeDLDownloader(QObject):
    ...
    def download(self, url, options):
        ...
        while self._proc_is_alive():
            ...
            #print("stdout: %s" % type(stdout))
            #byte to str
            if type(stdout) is bytes:
                stdout = stdout.decode("utf-8") 
            #print("stdout: %s, %s" % (type(stdout), stdout))
            if stdout:
